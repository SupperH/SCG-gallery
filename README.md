# *bug记录 & 改进*
## 待填坑
待增加个人信息页面 需要显示用户id 否则无法获取到用户id 也就无法使用团队空间的功能
改进： 引入断点续传，分片上传功能，引入cdn加速
目前只能删除缩略图和压缩后的webp格式图 原图不会删除实际还保存在cos中
...
## 2025/5/14
批量抓图优化 使用CompletableFuture 并根据核心数自定义线程池核心数的算法  
将批量接口改为异步，大幅度提高性能
...
目前使用了websocket+disruptor实现了协同编辑功能  
disruptor异步消息处理机制，将原有的同步消息分发逻辑改造为高效解耦的异步处理模型

WebSocket 与 Disruptor 的协作关系  
**层级          技术                        职责
网络层        WebSocket       负责接收客户端请求、建立连接、发送响应
业务层        Disruptor       负责异步处理消息、协调多线程消费、保障线程安全**  

具体协作流程如下：
1. 客户端通过 WebSocket 发送操作指令（如放大、缩小等）。
2. WebSocket 收到消息后，将消息封装为事件，并通过 PictureEditEventProducer 发布到 Disruptor 队列。
3. Disruptor 消费该事件，调用相应的业务逻辑（如 handlerEditActionMessage）。
4. 业务逻辑处理完成后，通过 WebSocket 广播给所有相关客户端。

也就是说 websocket只负责接收和发送消息，disruptor负责处理具体的业务逻辑，提升处理性能
